...
def get_valid_segments(segment_dir, min_duration=1.0):
    """
    segments 디렉토리 내의 segment_숫자.mp4 파일들 중
    - 길이가 N/A(빈파일) 또는 min_duration 미만이면 제외한다.
    - 유효한 세그먼트 경로 리스트와 인덱스 리스트를 반환한다.
    """
    segments = sorted([
        os.path.join(segment_dir, f)
        for f in os.listdir(segment_dir)
        if re.match(r"segment_\d+\.mp4", f)
    ], key=extract_number)
    valid_segments = []
    valid_indices = []
    for seg in segments:
        ...
        try:
            ...
            print(f"[SKIP] {os.path.basename(seg)} duration=N/A (빈파일 가능성) → 제외/삭제")
            if os.path.exists(seg) and os.path.getsize(seg) < 2048:  # 파일 용량 2KB 이하
                os.remove(seg)
                print(f"    → {seg} 삭제 완료")
            continue
...
def get_valid_segments(segment_dir, min_duration=1.0):
    ...
    segments = sorted([
        ...
        for f in os.listdir(segment_dir)
        if re.match(r"segment_\d+\.mp4", f)
    ], key=extract_number)  # segment_숫자만 필터링
    valid_segments = []
    valid_indices = []
    for seg in segments:
        # ffprobe로 길이 측정
...
def encode_segment_per_resolution(input_path, crf, max_rate, video_dir, segment_length):
    """
    segment_x.mp4의 내부 timestamp가 이미 누적이므로, 그대로 m4s로 인코딩
    """
    resolutions = {
        "360p":  ("640x360", 0),
        "480p":  ("854x480", 1),
    ...
    # max_rate 단위 변환
    if isinstance(max_rate, str) and max_rate.endswith('k'):
        rate_val = int(max_rate[:-1]) * 1000
    elif isinstance(max_rate, str) and max_rate.endswith('M'):
        rate_val = int(max_rate[:-1]) * 1000000
    else:
        rate_val = int(float(max_rate))
    ...
    if not match:
        raise ValueError(f"잘못된 세그먼트 이름: {segment_name}")
...
def split_video_to_segments_reencode(input_path, segment_dir, segment_length=10):
    """
    입력 mp4 전체를 '누적 pts'를 유지하며, 정확한 keyframe에 맞춰 10초 단위로 분할!
    각 segment_x.mp4의 내부 타임스탬프도 0, 10, 20...으로 누적됨
    """
    ...
    for i in range(num_segments):
        ...
        # ★ 실제 남은 길이만큼만 자르기 (마지막은 10초 미만 가능)
        actual_length = min(segment_length, total_duration - start)
        out_path = os.path.join(segment_dir, f"segment_{i}.mp4")
        cmd = [
            "ffmpeg", "-y",
            "-ss", str(start),  # -ss를 -i 앞에
            "-i", input_path,
            "-t", str(actual_length),  # ★여기서 실제 남은 길이 사용!
            "-c:v", "libx264",
            "-preset", "fast",
            "-crf", "23",
            "-r", "24",
            ...
            "-sc_threshold", "0",
            "-force_key_frames", f"expr:gte(t,n_forced*{segment_length})",
            "-copyts",                      # pts 누적(원본과 동일)
            "-avoid_negative_ts", "1",      # 음수 타임스탬프 방지
            "-an",
            out_path
        ]
        subprocess.run(cmd, check=True)
        segments.append(out_path)
        print(f"[디버그] segment_{i}.mp4 분할 완료 - start={start}s, length={actual_length}s")
...
for file in video_files:
    ...
    for idx, segment_path in enumerate(segments):
        ...
        if segment_name.startswith("segment_0"):
